...
while($bar=mysql_fetch_array($query)){	
	...
	while($i<$jml_ExRule){				
		...
		if($i==($jml_ExRule-1)){			
			...
			if($exRule[$i-1]=="---- "){
				echo "<font color='#920'><b>".$exRule[$i]."</b></font> <i>Maka prestasi = </i><strong>".$row_bar[0]." (".$id[$ll].")</strong>";
			}else if($exRule[$i-1]!="---- "){
				echo "<br>".$strip."<font color='#920'><b>".$exRule[$i]."</b></font> <i>Maka prestasi = </i><strong>".$row_bar[0]."  (".$id[$ll].")</strong>";
			}
		}
		//jika pertama
		else if($i==0){
