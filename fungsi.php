...
error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
	...
	function cek_nilaiAtribut($field , $kondisi){
		...
		$hasil = array();
		if($kondisi==''){
			$sql = mysql_query("SELECT DISTINCT($field) FROM data_training1");					
		}else{
			$sql = mysql_query("SELECT DISTINCT($field) FROM data_training1 WHERE $kondisi");					
		}
		$a=0;
	...
	function jumlah_data($kondisi){
		//sql
		if($kondisi==''){
			$sql = mysql_query("SELECT COUNT(*) FROM data_training1 $kondisi");	
		}else{
			$sql = mysql_query("SELECT COUNT(*) FROM data_training1 WHERE $kondisi");						
		}		
	...
	function hitung_gain($kasus , $atribut , $ent_all , $kondisi1 , $kondisi2 , $kondisi3 , $kondisi4 , $kondisi5){
		...
		if($kondisi3==''){
			$j_tinggi1 = jumlah_data("$data_kasus penerima_kip='Ya' AND $kondisi1");
			$j_rendah1 = jumlah_data("$data_kasus penerima_kip='Tidak' AND $kondisi1");
			$jml1 = $j_tinggi1 + $j_rendah1;
			$j_tinggi2 = jumlah_data("$data_kasus penerima_kip='Ya' AND $kondisi2");
			$j_rendah2 = jumlah_data("$data_kasus penerima_kip='Tidak' AND $kondisi2");
		...
		//untuk atribut 3 nilai atribut
			...
			$j_rendah2 = jumlah_data("$data_kasus penerima_kip='Tidak' AND $kondisi2");
			$jml2 = $j_tinggi2 + $j_rendah2;
			$j_tinggi3 = jumlah_data("$data_kasus penerima_kip='Ya' AND $kondisi3");
			$j_rendah3 = jumlah_data("$data_kasus penerima_kip='Tidak' AND $kondisi3");
		...
		//untuk atribut 4 nilai atribut
			...
			$j_rendah3 = jumlah_data("$data_kasus penerima_kip='Tidak' AND $kondisi3");
			$jml3 = $j_tinggi3 + $j_rendah3;
			$j_tinggi4 = jumlah_data("$data_kasus penerima_kip='Ya' AND $kondisi4");
			$j_rendah4 = jumlah_data("$data_kasus penerima_kip='Tidak' AND $kondisi4");
		...
		//untuk atribut 5 nilai atribut	
			...
			$j_rendah4 = jumlah_data("$data_kasus penerima_kip='Tidak' AND $kondisi4");
			$jml4 = $j_tinggi4 + $j_rendah4;
			$j_tinggi5 = jumlah_data("$data_kasus penerima_kip='Ya' AND $kondisi5");
			$j_rendah5 = jumlah_data("$data_kasus penerima_kip='Tidak' AND $kondisi5");
	...
	function hitung_rasio($kasus , $atribut , $gain , $nilai1 , $nilai2 , $nilai3 , $nilai4 , $nilai5){				
		...
		mysql_query("INSERT INTO aturan VALUES 
